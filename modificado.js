...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
            ...
            res.write(`Total de películas en cartelera: Tenés para elegir *${totalPelis}* películas \n`);
            res.write('----------------------------------- \n ');
            
            let títulos= [];
            movies.forEach(function(movie){
                títulos.push(movie.title)
            })

            títulos.sort();

            títulos.forEach(function(título){
            res.write(`*${título}\n\n`)
            })
            res.write('----------------------------------- \n ');
		...
		case '/mas-votadas':
            ...
            res.write('Total de películas: ');
            
            let masVotadas=movies.filter(function(movie){return movie.vote_average >= 7})//Acá puedo pasarle a push el objeto completo o la propiedad del objeto

			masVotadas.sort(function(a,b){
				return ((a.title<b.title)?-1:((a.title>b.title)?1:0));
			})

            res.write(`${masVotadas.length}\n`);
            res.write('----------------------------------- \n ');

            res.write('Raketing promedio:')
			let promedio=0;
			masVotadas.forEach(function(masVotada){
				promedio=promedio+masVotada.vote_average
			})

			promedio = promedio/masVotadas.length
            res.write(`${promedio.toFixed(2)}\n`);
            res.write('----------------------------------- \n ');

            res.write('LISTADO DE PELICULAS:\n**************************\n\n');
            masVotadas.forEach(function(masVotada){
            res.write(` >Título:${masVotada.title}\n >Raketing: *${masVotada.vote_average}*\n >Reseña: ${masVotada.overview}\n__________________________________\n`)
            });
